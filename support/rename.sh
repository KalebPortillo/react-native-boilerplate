...
filesToModify=$(grep -riIl 'boilerplate' --exclude='rename.sh' $appRoot/*)
filesToRename=$(find "${appRoot}/ios" "${appRoot}/android" -type f -ipath '*boilerplate*')

# replace strings in files
for fileToModify in $filesToModify; do
  sed -i.bak "s/boilerplate/${newAppName}/g;s/boilerplate/${newLowerCaseName}/g" $fileToModify
done
...
for fileToRename in $filesToRename; do
  newName=$(echo $fileToRename | sed "s/boilerplate/$newAppName/g;s/boilerplate/$newLowerCaseName/g")
  mkdir -p $(dirname "$newName") && $mvCmd "$fileToRename" "$newName"
done

# remove leftover empty directories
rmdir -p $(find "$appRoot" -ipath '*boilerplate*' -type d) 2>/dev/null
